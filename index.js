...
app.post('/api/posts', async (req, res) => { const { author, title, content } = req.body; if (!author || !title || !content) { return res.status(400).json({ success: false, message: '작성자, 제목, 내용을 모두 입력해주세요.' }); } try { const queryText = 'INSERT INTO posts(author, title, content) VALUES($1, $2, $3) RETURNING id'; const result = await pool.query(queryText, [author, title, content]); res.status(201).json({ success: true, message: '게시글이 성공적으로 등록되었습니다.', postId: result.rows[0].id }); } catch (err) { console.error('게시글 작성 중 오류 발생:', err); res.status(500).json({ success: false, message: '서버 오류로 글 등록에 실패했습니다.' }); } });
app.get('/api/posts', async (req, res) => { const page = parseInt(req.query.page || '1', 10); const limit = parseInt(req.query.limit || '10', 10); const offset = (page - 1) * limit; const searchTerm = req.query.search || ''; let whereClause = ''; const values = [limit, offset]; if (searchTerm) { whereClause = `WHERE title ILIKE $3 OR content ILIKE $3 OR author ILIKE $3`; values.push(`%${searchTerm}%`); } try { const countQuery = `SELECT COUNT(*) FROM posts ${whereClause}`; const dataQuery = `SELECT id, author, title, created_at FROM posts ${whereClause} ORDER BY created_at DESC LIMIT $1 OFFSET $2`; const countResult = await pool.query(countQuery, searchTerm ? [`%${searchTerm}%`] : []); const dataResult = await pool.query(dataQuery, values); const totalItems = parseInt(countResult.rows[0].count, 10); const totalPages = Math.ceil(totalItems / limit); res.status(200).json({ data: dataResult.rows, pagination: { totalItems, totalPages, currentPage: page, limit } }); } catch (err) { console.error('게시글 목록 조회 중 오류 발생:', err); res.status(500).json({ success: false, message: '목록 조회에 실패했습니다.' }); } });
app.get('/api/posts/:id', async (req, res) => { const { id } = req.params; try { const queryText = 'SELECT * FROM posts WHERE id = $1'; const { rows } = await pool.query(queryText, [id]); if (rows.length > 0) { res.status(200).json(rows[0]); } else { res.status(404).json({ success: false, message: '해당 게시글을 찾을 수 없습니다.' }); } } catch (err) { console.error('게시글 상세 조회 중 오류 발생:', err); res.status(500).json({ success: false, message: '게시글을 불러오는 데 실패했습니다.' }); } });
...
app.get('/api/admin/notices', verifyToken, async (req, res) => { const page = parseInt(req.query.page || '1', 10); const limit = parseInt(req.query.limit || '10', 10); const offset = (page - 1) * limit; const searchTerm = req.query.search || ''; let whereClause = ''; const values = [limit, offset]; if (searchTerm) { whereClause = `WHERE title ILIKE $3 OR content ILIKE $3`; values.push(`%${searchTerm}%`); } try { const countQuery = `SELECT COUNT(*) FROM notices ${whereClause}`; const dataQuery = `SELECT id, title, created_at FROM notices ${whereClause} ORDER BY created_at DESC LIMIT $1 OFFSET $2`; const countResult = await pool.query(countQuery, searchTerm ? [`%${searchTerm}%`] : []); const dataResult = await pool.query(dataQuery, values); const totalItems = parseInt(countResult.rows[0].count, 10); const totalPages = Math.ceil(totalItems / limit); res.status(200).json({ data: dataResult.rows, pagination: { totalItems, totalPages, currentPage: page, limit, }, }); } catch (err) { console.error('관리자 공지사항 목록 조회 중 오류 발생:', err); res.status(500).json({ success: false, message: '목록 조회에 실패했습니다.' }); } });
...
// --- 관리자 삭제 API (새로 추가) ---
app.delete('/api/admin/consultations/:id', verifyToken, async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query('DELETE FROM consultations WHERE id = $1', [id]);
    res.status(200).json({ success: true, message: '상담글이 성공적으로 삭제되었습니다.' });
  } catch (err) {
...
app.delete('/api/admin/posts/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ success: true, message: '게시글이 성공적으로 삭제되었습니다.' });
  } catch (err) {
...
app.get('/api/admin/posts', verifyToken, async (req, res) => {
  ...
  if (searchTerm) {
    whereClause = `WHERE title ILIKE $3 OR author ILIKE $3 OR content ILIKE $3`;
    values.push(`%${searchTerm}%`);
  }
  try {
    const countQuery = `SELECT COUNT(*) FROM posts ${whereClause}`;
    const dataQuery = `SELECT id, title, author, created_at FROM posts ${whereClause} ORDER BY created_at DESC LIMIT $1 OFFSET $2`;
    ...
    console.error('관리자 게시글 목록 조회 중 오류 발생:', err);
