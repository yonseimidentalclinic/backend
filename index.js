...
const createTables = async () => {
    ...
    const consultationsTableQuery = `CREATE TABLE IF NOT EXISTS consultations (id SERIAL PRIMARY KEY, author VARCHAR(100) NOT NULL, password VARCHAR(255) NOT NULL, title VARCHAR(255) NOT NULL, content TEXT NOT NULL, is_secret BOOLEAN DEFAULT false, created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP);`;
    try {
...
app.post('/api/admin/login', (req, res) => {
  ...
  if (password === process.env.ADMIN_PASSWORD) {
    const token = jwt.sign({ isAdmin: true }, process.env.JWT_SECRET, { expiresIn: '3h' });
...
// --- 관리자 대시보드 데이터 API (새로 추가) ---
// verifyToken 미들웨어를 통과해야만 이 API를 실행할 수 있습니다.
app.get('/api/admin/dashboard', verifyToken, async (req, res) => {
  try {
    // 최신 상담 5개와 최신 공지 5개를 동시에 가져옵니다.
    const consultationQuery = 'SELECT id, title, author, created_at FROM consultations ORDER BY created_at DESC LIMIT 5';
    const noticeQuery = 'SELECT id, title, created_at FROM notices ORDER BY created_at DESC LIMIT 5';

    const [consultationResult, noticeResult] = await Promise.all([
      pool.query(consultationQuery),
      pool.query(noticeQuery)
    ]);

    res.status(200).json({
      latestConsultations: consultationResult.rows,
      latestNotices: noticeResult.rows,
    });
  } catch (err) {
    console.error('대시보드 데이터 조회 중 오류 발생:', err);
    res.status(500).json({ success: false, message: '데이터를 불러오는 데 실패했습니다.' });
  }
});
