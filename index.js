// =================================================================
// 연세미치과 홈페이지 백엔드 최종 코드 (CORS 및 API 경로 수정)
// 최종 업데이트: 2025년 7월 18일
// 주요 개선사항:
// 1. 중복된 CORS 설정 제거로 인증 문제 해결
// 2. 관리자 공지사항 관리를 위한 목록 및 상세 조회 API 추가
// =================================================================
...
// 수정된 CORS 옵션을 Express 앱에 적용합니다. 이 설정 하나만 유지합니다.
app.use(cors(corsOptions));

app.set('trust proxy', true); // Render.com 프록시 환경에서 정확한 IP를 얻기 위해 필요
// app.use(cors()); // <<<< 문제의 원인이었던 중복 코드 삭제
...
async function initializeDatabase() {
  ...
  try {
    ...
    const createAboutContentTable = `CREATE TABLE IF NOT EXISTS about_content (id INT PRIMARY KEY DEFAULT 1, title TEXT, subtitle TEXT, content TEXT, image_data TEXT, updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW());`;
    const insertDefaultAboutContent = `INSERT INTO about_content (id, title, subtitle, content) VALUES (1, '연세미치과 이야기', '환자 한 분 한 분의 건강한 미소를 위해, 저희는 보이지 않는 곳까지 정성을 다합니다.', '연세미치과는 단순히 아픈 곳을 치료하는 것을 넘어, 환자분들의 삶의 질을 높이는 것을 목표로 합니다.') ON CONFLICT (id) DO NOTHING;`;
    const createClinicPhotosTable = `CREATE TABLE IF NOT EXISTS clinic_photos (id SERIAL PRIMARY KEY, caption VARCHAR(255), image_data TEXT NOT NULL, display_order SERIAL, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW());`;
    const createPostCommentsTable = `CREATE TABLE IF NOT EXISTS post_comments (id SERIAL PRIMARY KEY, post_id INTEGER NOT NULL REFERENCES posts(id) ON DELETE CASCADE, author VARCHAR(100) NOT NULL, password VARCHAR(255) NOT NULL, content TEXT NOT NULL, likes INTEGER DEFAULT 0, tags TEXT, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW());`;
    ...
    const createReservationsTable = `CREATE TABLE IF NOT EXISTS reservations (id SERIAL PRIMARY KEY, patient_name VARCHAR(100) NOT NULL, phone_number VARCHAR(100) NOT NULL, desired_date DATE NOT NULL, desired_time VARCHAR(50) NOT NULL, notes TEXT, status VARCHAR(50) DEFAULT 'pending', created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW());`;
    const createCasePhotosTable = `CREATE TABLE IF NOT EXISTS case_photos (id SERIAL PRIMARY KEY, title VARCHAR(255) NOT NULL, category VARCHAR(100), description TEXT, before_image_data TEXT, after_image_data TEXT, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW());`;
    const createFaqsTable = `CREATE TABLE IF NOT EXISTS faqs (id SERIAL PRIMARY KEY, category VARCHAR(100) NOT NULL, question TEXT NOT NULL, answer TEXT NOT NULL, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW());`;
    const createBlockedSlotsTable = `CREATE TABLE IF NOT EXISTS blocked_slots (id SERIAL PRIMARY KEY, slot_date DATE NOT NULL, slot_time VARCHAR(50) NOT NULL, UNIQUE(slot_date, slot_time));`;
    const createReviewsTable = `CREATE TABLE IF NOT EXISTS reviews (id SERIAL PRIMARY KEY, patient_name VARCHAR(100) NOT NULL, rating INT NOT NULL CHECK (rating >= 1 AND rating <= 5), content TEXT NOT NULL, is_approved BOOLEAN DEFAULT FALSE, admin_reply TEXT, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW());`;
    const createAdminLogsTable = `CREATE TABLE IF NOT EXISTS admin_logs (id SERIAL PRIMARY KEY, action VARCHAR(100) NOT NULL, ip_address VARCHAR(100), created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW());`;
...
// [핵심 추가] 공지사항 상세 (관리자용)
app.get('/api/admin/notices/:id', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/admin/blocked-slots', authenticateToken, async (req, res) => { const { slotDate, slotTime } = req.body; try { await pool.query('INSERT INTO blocked_slots (slot_date, slot_time) VALUES ($1, $2) ON CONFLICT DO NOTHING', [slotDate, slotTime]); res.status(201).send(); } catch (err) { console.error('예약 불가 시간 추가 오류:', err); res.status(500).send('서버 오류'); } });
app.delete('/api/admin/blocked-slots', authenticateToken, async (req, res) => { const { slotDate, slotTime } = req.body; try { await pool.query('DELETE FROM blocked_slots WHERE slot_date = $1 AND slot_time = $2', [slotDate, slotTime]); res.status(204).send(); } catch (err) { console.error('예약 불가 시간 삭제 오류:', err); res.status(500).send('서버 오류'); } });
...
app.post('/api/reviews', async (req, res) => { const { patientName, rating, content } = req.body; try { const result = await pool.query('INSERT INTO reviews (patient_name, rating, content) VALUES ($1, $2, $3) RETURNING *', [patientName, rating, content]); res.status(201).json(toCamelCase(result.rows)[0]); } catch (err) { console.error('후기 작성 중 오류:', err); res.status(500).send('서버 오류'); } });
app.get('/api/admin/reviews', authenticateToken, async (req, res) => { try { const result = await pool.query('SELECT * FROM reviews ORDER BY created_at DESC'); res.json(toCamelCase(result.rows)); } catch (err) { console.error('관리자 후기 조회 오류:', err); res.status(500).send('서버 오류'); } });
app.put('/api/admin/reviews/:id/approve', authenticateToken, async (req, res) => { const { isApproved } = req.body; try { const result = await pool.query('UPDATE reviews SET is_approved = $1 WHERE id = $2 RETURNING *', [isApproved, req.params.id]); if (result.rows.length === 0) return res.status(404).send('후기를 찾을 수 없습니다.'); res.json(toCamelCase(result.rows)[0]); } catch (err) { console.error('후기 승인 오류:', err); res.status(500).send('서버 오류'); } });
app.post('/api/admin/reviews/:id/reply', authenticateToken, async (req, res) => { const { reply } = req.body; try { const result = await pool.query('UPDATE reviews SET admin_reply = $1 WHERE id = $2 RETURNING *', [reply, req.params.id]); if (result.rows.length === 0) return res.status(404).send('후기를 찾을 수 없습니다.'); res.json(toCamelCase(result.rows)[0]); } catch (err) { console.error('답글 작성 오류:', err); res.status(500).send('서버 오류'); } });
app.delete('/api/admin/reviews/:id', authenticateToken, async (req, res) => { try { const result = await pool.query('DELETE FROM reviews WHERE id = $1', [req.params.id]); if (result.rowCount === 0) return res.status(404).send('후기를 찾을 수 없습니다.'); res.status(204).send(); } catch (err) { console.error('후기 삭제 오류:', err); res.status(500).send('서버 오류'); } });
