...
// --- 관리자 공지사항 목록 API (페이지네이션 + 검색) ---
app.get('/api/admin/notices', verifyToken, async (req, res) => {
  const page = parseInt(req.query.page || '1', 10);
  const limit = parseInt(req.query.limit || '10', 10);
  const offset = (page - 1) * limit;
  const searchTerm = req.query.search || '';
  ...
  if (searchTerm) {
    whereClause = `WHERE title ILIKE $3 OR content ILIKE $3`;
    values.push(`%${searchTerm}%`);
  }

  try {
    const countQuery = `SELECT COUNT(*) FROM notices ${whereClause}`;
    const dataQuery = `SELECT id, title, created_at FROM notices ${whereClause} ORDER BY created_at DESC LIMIT $1 OFFSET $2`;
    ...
    console.error('관리자 공지사항 목록 조회 중 오류 발생:', err);
    res.status(500).json({ success: false, message: '목록 조회에 실패했습니다.' });
  }
});
